...
Las definiciones de etiquetas se consolidan en `src/content/tags/{tagId}.json`, con cada etiqueta teniendo un campo `i18n.name`. Esto traslada la fuente de verdad para las traducciones de etiquetas de los archivos JSON de traducción a la colección de etiquetas.
...
Las tarjetas de artículos, la barra lateral, el índice de etiquetas y el detalle del artículo hacen referencia a la colección de etiquetas para cambiar los nombres visibles, asegurando que todos los tags se muestren de forma unificada en el idioma correspondiente.

## Datos de autor multilingües

Los nombres, las biografías y las listas de habilidades de los autores también cambian según el idioma. Se añadió un campo `i18n` a `src/content/authors/{authorId}.json` para almacenar las traducciones de cada idioma.
