...
As definições de tags são consolidadas em `src/content/tags/{tagId}.json`, com cada tag tendo um campo `i18n.name`. Isso move a fonte de verdade para as traduções de tags dos arquivos JSON de tradução para a coleção de tags.
...
Cards de artigos, barra lateral, índice de tags e detalhe do artigo fazem referência à coleção de tags para alternar os nomes visíveis, garantindo que todas as tags sejam exibidas de forma unificada no idioma apropriado.

## Dados de autor multilíngues

Os nomes, as biografias e as listas de habilidades dos autores também mudam conforme o idioma. Um campo `i18n` foi adicionado ao `src/content/authors/{authorId}.json` para armazenar as traduções de cada idioma.
