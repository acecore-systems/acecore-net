...
태그 정의는 `src/content/tags/{tagId}.json`에 집약되며, 각 태그는 `i18n.name` 필드를 가집니다. 이를 통해 태그 번역의 source of truth를 번역 JSON 파일에서 태그 collection으로 이동합니다.
...
글 카드, 사이드바, 태그 인덱스, 글 상세에서 모두 tags collection을 참조하여 표시 이름을 전환하며, 모든 태그 표시가 해당 언어로 통일됩니다.
