...
Acecore では、コーポレートサイト（acecore-net）と事業計画ドキュメント（business-plan）の2つのプロジェクトを運用しています。今回、事業計画ドキュメントを VitePress から Starlight に移行し、両プロジェクトのフレームワークを Astro に統一しました。

## なぜフレームワークを統一するのか

2つのプロジェクトで異なるフレームワークを使っていると、以下の問題が発生します：
...
フレームワークの統一により、`astro.config.mjs` の設定パターンや Cloudflare Pages のデプロイ設定を両プロジェクトで共有できるようになりました。

## まとめ

フレームワーク統一は「今すぐ必要」ではなくても、運用が長くなるほど効いてくる施策です。VitePress から Starlight への移行自体は数時間で完了し、Mermaid の CDN 化はむしろプラグイン管理からの解放というメリットがありました。複数プロジェクトを運用している方は、技術スタックの統一を検討してみてください。
