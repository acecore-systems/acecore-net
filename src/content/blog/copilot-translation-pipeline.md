...
- 著者情報の翻訳元: `src/data/authors.json`
- 著者情報の翻訳先: `src/data/authors.json` の `i18n`
...
- `main` への push を監視する
- `src/content/blog/*.md` と `src/data/authors.json` だけを対象にする
...
このとき、記事用と著者情報用で `custom_instructions` を分けておくと精度が安定します。著者情報では `src/data/authors.json` の `i18n` だけを触る、記事では `src/content/blog/{locale}/` に同名ファイルを作る、というルールを明示しておくと事故が減ります。
