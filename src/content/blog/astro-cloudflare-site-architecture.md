...
faq:
  title: よくある質問
  items:
    - question: この記事は単体記事と何が違いますか？
      answer: '単体記事は各機能の実装詳細を扱います。この記事は、AI、CMS、多言語、フォーム、コメント、検索を同じ公式サイトの中でどう分担させるかを整理する入口です。'
...
ここ数本の記事で、問い合わせAI、Sveltia CMS、多言語ブログ、サービスCTA、AI回答のMarkdownリンク、安全なコメント機能をそれぞれ記事化しました。

単体機能の記事としてはかなり揃ってきました。では、それらをどういう順番で読み、どういう全体像として他サイトへ転用すればよいのか。

この記事では、Acecore公式サイトを **Astro + Cloudflareを土台にした機能拡張型の静的サイト** として整理します。単なるリンク集ではなく、機能を足すときの境界設計としてまとめます。
...
- [Astroサイトの品質改善ガイド](/blog/website-improvement-batches/)
- [Astroサイトの品質改善ガイド 続編](/blog/website-improvement-final-batch/)
- [Astro 6 サイトを9言語対応に](/blog/astro-i18n-blog-translation/)
- [Acecore公式サイトをリニューアルしました](/blog/website-renewal/)

ただし、これらはパフォーマンス、SEO、アクセシビリティ、多言語化、リニューアル告知が中心です。

今回必要だったのは、最近追加した **問い合わせAI、CMS運用、多言語翻訳、フォーム導線、コメント機能** をまとめる記事です。既存記事とは役割が違います。
...
## 9. 記事化するときは、機能単体と総集編を分ける

今回の一連の記事で分かったのは、単体機能の記事と総集編の記事は役割が違うということです。

単体機能の記事は、実装で困っている人に刺さります。

- 問い合わせAIを入れたい
- Sveltia CMSを入れたい
- 多言語ブログを運用したい
- コメント機能をCloudflareだけで作りたい
- AI回答のMarkdownリンクを安全に描画したい

一方で、総集編は「どこから読むべきか」「どう組み合わせるべきか」を渡します。

機能が増えた公式サイトは、読者から見ると全体像がつかみにくくなります。そこで、総集編がハブになります。

## おすすめの読み順

これから同じような構成を他サイトへ入れるなら、読む順番はこうです。

1. [Sveltia CMS導入ガイド](/blog/cms-selection-and-turnstile/)
2. [Sveltia CMSで多言語ブログを運用する方法](/blog/copilot-translation-pipeline/)
3. [Astroサイトに問い合わせAIチャットを組み込む技術設計](/blog/astro-ai-contact-chat/)
4. [AIチャット回答のMarkdownリンクを安全に描画する実装設計](/blog/ai-chat-markdown-link-safety/)
5. [サービスCTAから問い合わせフォームへ文脈を引き継ぐ技術設計](/blog/service-cta-contact-prefill/)
6. [CloudflareだけでAstroブログにコメント機能を作る方法](/blog/cloudflare-only-blog-comments/)
...
Astroは公開HTMLを作る。Cloudflareは配信と小さなAPI境界を受け持つ。Sveltia CMSは日本語sourceを編集する。翻訳はPRで作る。AIは問い合わせ前の整理に使う。コメントはCloudflare内に閉じる。検索対象はレビュー済みコンテンツに絞る。

このようにレイヤーを分けると、公式サイトは単なる会社案内ではなく、更新、翻訳、相談、交流まで扱える運用基盤になります。

単体機能を記事化したあとに総集編を作る価値はここにあります。

各機能の実装記事をつなげることで、「どう作ったか」だけでなく、「なぜその形にしたか」まで伝えられるようになります。
