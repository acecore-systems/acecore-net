...
На стороне GitHub Actions после обнаружения изменения задача отправляется в API агента кодирования Copilot.

Нужно сделать 2 вещи.

1. Добавить `COPILOT_AGENT_TOKEN` в качестве секрета репозитория
2. Вызвать API задания Copilot для каждого изменённого пути к источнику

Концептуально вы передаёте заголовок и описание проблемы в API задания Copilot.
...
Когда не проходят через issues, управление дублированием перемещается на сторону PR.

Подход прост: перед созданием задачи сделайте следующее.

1. Вывести маркер `translation-source:` из пути к источнику
2. Поискать в GitHub открытые PR с тем же маркером
3. Если открытый PR существует, не создавать задачу
4. Если открытый PR не существует, создать задачу Copilot для PR перевода

Причина включения пути к источнику в тело PR заключается в том, что просмотр только изменённых файлов PR перевода затрудняет надёжный обратный поиск исходного японского файла. **Явное указание пути к источнику в виде маркера** позволяет не создавать несколько PR переводов для одной статьи.
