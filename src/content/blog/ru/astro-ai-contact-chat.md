...
На сайте Acecore AI-чат для обращений был добавлен в статическую архитектуру Astro + Cloudflare Pages. Основная реализация находится в [PR с AI для контактов и CMS-ограниченным потоком переводов](https://github.com/acecore-systems/acecore-net/pull/98). Затем безопасный рендеринг Markdown-ссылок был доработан в [отдельном PR](https://github.com/acecore-systems/acecore-net/pull/99).
...
Эта статья только о техническом дизайне AI-чата. Передача контекста страницы услуги в форму — отдельная тема.
