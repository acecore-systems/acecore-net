...
Определения тегов консолидированы в `src/content/tags/{tagId}.json`, где каждый тег имеет поле `i18n.name`. Это перемещает источник истины для переводов тегов из файлов JSON переводов в коллекцию тегов.
...
Карточки статей, боковая панель, индекс тегов и детали статьи — все обращаются к коллекции тегов для переключения отображаемых названий, обеспечивая единообразное отображение всех тегов на соответствующем языке.

## Мультиязычные данные авторов

Имена, биографии и списки навыков авторов также переключаются в зависимости от языка. В `src/content/authors/{authorId}.json` добавлено поле `i18n` для хранения переводов на каждый язык.
