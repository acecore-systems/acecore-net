...
6. 著者・タグは「著者」「タグ」から編集
7. CMS 経由の日本語ソース編集のみ、Copilot translation PR task で翻訳へ反映

#### キャンペーン告知の運用

- 共通の告知バナーは「ページ・サイト文言」→「共通UI・ナビ」→「告知バナー」から編集します。
- 告知バナーは `表示する`、`表示開始日時`、`表示終了日時` で公開期間を制御します。日時は日本時間の `YYYY-MM-DDTHH:mm` として扱われます。
- ページ内のキャンペーン・募集・残数表示は「ページ・サイト文言」→「共通UI・ナビ」→「キャンペーン通知」から編集します。
- キャンペーン通知は `表示位置` が `CampaignNotice` コンポーネントの `placement` と一致する場所に表示されます。例: Schools ページのヒーロー下は `schools-after-hero`。
- キャンペーン通知ではボタン文言、リンクURL、表示トーン、アイコン、残数などの指標、表示開始/終了日時を CMS から変更できます。
