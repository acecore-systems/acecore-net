...
4. AI に issue を処理させ、`src/content/blog/{locale}/` を更新する

著者情報とタグ定義の `i18n` は Pages CMS では非表示にしてあり、必要なときだけ manual dispatch で翻訳 issue を起こす前提です。
...
- blog 記事は frontmatter だけの変更では再起票せず、Markdown 本文が変わったときだけ issue を作成または更新
- authors/tags の翻訳 issue は `workflow_dispatch` で `include_non_blog_sources=true` を指定したときだけ起票する
...
### issue 自動クローズ workflow

- Workflow: `.github/workflows/close-translation-issues.yml`
- Script: `scripts/close-translation-issues.mjs`
- 対象は merge 済みの translation PR のみ
- PR の変更ファイルと PR body 内の source path から `translation-source:` マーカーを逆引きし、対応する open issue を自動でコメント付きクローズする
- `workflow_dispatch` で過去の merged PR に対しても手動実行できる
